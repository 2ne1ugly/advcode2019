...
int main() {
	...
	mvprintw(2, 0, "%5d", found);
	getch();

	queue<pair<int, int>> oxyQueue;
	queue<pair<int, int>> oxyNextQueue;
	map[oxy] = 1;
	oxyQueue.push(oxy);
	depth = -1;
	while(!oxyQueue.empty()) {
		mvprintw(0, 0, "%d", depth);
		while (!oxyQueue.empty()) {
			pair<int, int> pos = oxyQueue.front();
			oxyQueue.pop();
			...
			mvaddch(pos.second + offset, pos.first + offset * 3, 'O');
			refresh();
			oxyNextQueue.push({pos.first, pos.second + 1});
			oxyNextQueue.push({pos.first, pos.second - 1});
			oxyNextQueue.push({pos.first - 1, pos.second});
			oxyNextQueue.push({pos.first + 1, pos.second});
		}
		oxyQueue.swap(oxyNextQueue);
		depth++;
	}
	mvprintw(2, 0, "%5d", depth - 1);
	getch();
